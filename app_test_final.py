...
# ── Gesture ID → Arduino command character ──────────────────────────────────
GESTURE_COMMAND_MAP = {
    0: 'F',   # Class 0  →  Stop
    1: 'S',   # Class 1  →  Forward
    2: 'B',   # Class 2  →  Backward
    3: 'R',
}
# ────────────────────────────────────────────────────────────────────────────
...
def send_command(ser, command_char):
    """Send a single character command over serial, with a newline terminator."""
    if ser is not None:
...
def main():
    ...
    last_command = None  # ← Track last sent command to avoid serial spam
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if command_char and command_char != last_command:
                    send_command(ser, command_char)
                    last_command = command_char
                    print(f"Gesture ID {hand_sign_id} → Sent '{command_char}'")
                # ────────────────────────────────────────────────────────────

                if hand_sign_id == 0:  # "Not applicable" equivalent
            ...
            if last_command != 'S':
                send_command(ser, 'S')
                last_command = 'S'
                print("No hand detected → Sent 'S' (Stop)")
            point_history.append([0, 0])
    ...
    if ser is not None:
        send_command(ser, 'S')  # Safety stop on exit
        ser.close()
    cap.release()
    cv.destroyAllWindows()


# ── All original helper functions below (unchanged) ─────────────────────────
